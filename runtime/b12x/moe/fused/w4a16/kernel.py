...
from b12x.cute.fp4 import (
    ...
    packed_dequant_trellis3_to_bfloat2x4,
    nf3_codebook_pools,
...
def _shared_memory_footprint(
    *,
    ...
    if weight_layout in ("nf3_2p1", "trellis3_t256"):
        # NF3 and Trellis256 both store 12 bytes per 32-code unit, so their B
        # stage is 3/4 of the packed int4 footprint.
...
class W4A16GemmKernel:
    def __init__(
        ...
        *,
        ...
        if weight_layout == "trellis3_t256":
            # TRELLIS-3.0 PRODUCTION 256-weight tile (Option 1, NO re-quant).
            # Stages the exl3 24-u32 tile per warp; each lane funnel-extracts the
            # SAME two 32-bit windows exl3 dq8<3,1,4> reads (t_offset=lane*8) and
            # runs the UNCHANGED packed_dequant_trellis3 primitive.  The decode is
            # bit-exact to ext.reconstruct on the real checkpoint (probe oracle).
            # Reuses the e4m3_k32 scale-format plumbing (scales ignored by decode).
            if scale_format != "e4m3_k32":
                raise ValueError(
            ...
            if element_dtype != "bf16":
                raise ValueError(
                    "trellis3_t256 W4A16 weights are bf16-activation only (v1)"
                )
        ...
        # TRELLIS-3.0 PRODUCTION 256-weight tile (Option 1). Stages the exl3
        # 24-u32 tile per warp (NOT the 12-byte per-thread ring): each lane reads
        # the two tile words dq8<3,1,4> needs and funnels them in-kernel.  Own
        # staging + register-read path; the (2,4) B register bundle carries the
        # 4 jj-tiles' (a,b) word pairs [a0..a3, b0..b3].
        self.weight_layout_trellis256 = weight_layout == "trellis3_t256"
        # Projection-major FC1 backing [2,E,K/16,Nhalf/16,24u32] lets the
        # trellis megakernel and exl3 gate/up pointer tables share one physical
        # copy.  FC2 always receives w13_layout="packed" and keeps the admitted
        # native [E,K/16,N/16,24u32] address map.
        ...
        self.stages = int(pipeline_stages)
        if self.stages < 4 or self.stages > 8:
            raise ValueError(f"unsupported W4A16 pipeline_stages={self.stages}")
        self.cp_async_wait_groups = self.stages - 2
        if self.stages > _STAGES:
            # This mainloop preloads the next pipe's registers before the
            # lookahead wait point.  Use a correctness-first full wait for
            # widened rings; the oracle decides whether this lane is worth
            # promoting.
            self.cp_async_wait_groups = 0
        self.l2_prefetch = bool(l2_prefetch)
        if self.l2_prefetch and not self.weight_layout_nf3:
            raise ValueError("W4A16 L2 prefetch lane is NF3-only")
        ...
        self.b_unit_bytes = 12 if self.b_region_12byte else 16
        self.b_sh_stage_bytes = self.b_sh_stage * self.b_unit_bytes
        if self.b_region_12byte:
        ...
        if self.shared_words * 4 > int(max_shared_mem):
            raise ValueError(
                ...
                f"(stages={self.stages}, layout={self.weight_layout})"
            )
                    ...
                    for kk in cutlass.range_constexpr(self.b_sh_wr_iters):
                        ...
                        for jj in cutlass.range_constexpr(4):
                            if cutlass.const_expr(self.weight_layout_trellis256):
                                # TRELLIS-256 (PRODUCTION tile): the register read
                                # already funnel-extracted this jj-tile's two 32-bit
                                # windows (win_a, win_b) into b_scale_cur[0, jj] /
                                # b_scale_cur[1, jj].  Feed them to the UNCHANGED
                                # primitive; NO per-weight scale.
    ...
    def _load_b_scale_registers(
        ...
        if cutlass.const_expr(self.weight_layout == "modelopt"):
            ...
            # B region equals the stock staged-unit index. Each unit is a
            # 12-byte (lo0, lo1, hi) triple; three scalar u32 loads over stride-3
            # words are bank-conflict free (gcd(3, 32) == 1).
    ...
    @cute.jit
    def _trellis256_lane_geom(self, lane: Int32):
        # dq8<3,1,4> per-lane geometry over the 24-u32 (256-weight) tile.
        # i0 = (24*lane + 755) // 32 in [23, 46]; a-word = i0 % 24, b-word =
        # (a-word + 1) % 24; s2 = ((lane%4)+1)*8 mod 32 in {8,16,24,0}. Branchless
        # single-subtract %24 (i0 < 48) and mask-based %32.
        i0 = (Int32(24) * lane + Int32(755)) >> Int32(5)
        ia = i0 - Int32(24) * (i0 >= Int32(24)).to(Int32)
        ib = ia + Int32(1)
        ib = ib - Int32(24) * (ib >= Int32(24)).to(Int32)
        s2 = (((lane & Int32(3)) + Int32(1)) << Int32(3)) & Int32(31)
        return ia, ib, s2

    @cute.jit
    def _trellis_funnel256(self, a: Uint32, b: Uint32, s2: Int32):
        # 256-tile funnel = ((a<<32)|b) >> s2 (win_a) and >> (s2+12) (win_b), low
        # 32 bits.  a = tile[i0%24] (high word), b = tile[(i0+1)%24] (low word),
        # matching exl3 dq8 fshift(b, a, s2).  s2 is a runtime per-lane shift, so
        # the merge/shift use Int64 (bit-preserving; the primitive reads only the
        # low <=25 bits of each window so any sign fill in the ignored high bits is
        # harmless).  Reproduces the probe-verified constexpr funnel bit-for-bit.
        merged = (Int64(a) << Int64(32)) | Int64(b)
        win_a = Uint32(merged >> Int64(s2))
        win_b = Uint32(merged >> (Int64(s2) + Int64(12)))
        return win_a, win_b
    ...
    def _scaled_dequant_b_fragment_trellis256(
        ...
        # PRODUCTION 256-tile decode: the two funnel windows (pre-extracted per
        # (lane, jj-tile) in the register read) feed the UNCHANGED primitive.
        # Output element order matches the nf3/packed/L32 arms exactly:
        #   o0 -> frag[0,0]  o1 -> frag[0,1]  o2 -> frag[1,0]  o3 -> frag[1,1].
        # NO per-weight scale (unit-variance Gaussian codebook).
        o0, o1, o2, o3 = packed_dequant_trellis3_to_bfloat2x4(win_a, win_b)
        frag[0, 0] = o0
        frag[0, 1] = o1
        frag[1, 0] = o2
        frag[1, 1] = o3
    ...
    def _load_b_registers_trellis256(
        ...
        # PRODUCTION 256-tile per-warp register read.  The pipe's B SMEM region
        # holds the CTA's native exl3 tiles laid out [kt_local, nt16_local, 24u32]
        # contiguous (kt_local in 0..cta_k_blocks-1, nt16_local in
        # 0..cta_n_blocks-1).  This warp owns K16-row cur_group_id = b_sh_wr_iters
        # *warp_row + kk and the four CONSECUTIVE N16-tiles nt16_local = 4*w_n + jj
        # (jj = 0..3), so its four jj-tiles are a contiguous 96-u32 span.  Each
        # lane reads the SAME two tile words exl3 dq8<3,1,4> needs (ia, ib at
        # t_offset = lane*8) and funnels them.  The resulting (lane, jj) -> (k, n)
        # placement is IDENTICAL to the proven NF3/L32 mma_bpos (N16-tile
        # base_n64*4+jj, K16-row cur_group_id; within-tile n = lane//4 (+8 for
        # frag[1,*]), k = (lane%4)*2 + {0,1,8,9}) -- so the funnel256 output feeds
        # the shared mma with NO permutation.  Returned 8-tuple maps to
        # regs[0,0..3] = win_a_{0..3}, regs[1,0..3] = win_b_{0..3}.
        lane = tid & Int32(31)
        warp_id = tid >> Int32(5)
        warp_row = warp_id // Int32(self.tb_n_warps)
        w_n = warp_id % Int32(self.tb_n_warps)
        kt_local = Int32(self.b_sh_wr_iters) * warp_row + kk
        ia, ib, s2 = self._trellis256_lane_geom(lane)
        b_region = (
            smem_base
            + Int32(self.sh_b_off * 16)
            + pipe * Int32(self.b_sh_stage_bytes)
        )
        # tile_base_u32(jj) = (kt_local*cta_n_blocks + 4*w_n + jj) * 24 (u32).
        base_u32 = (
    ...
    def _stage_k_tile_async(
        ...
        if cutlass.const_expr(self.weight_layout_trellis256):
            # TRELLIS-256 staging: consume the NATIVE exl3 checkpoint tensor
            # [E, K/16, N/16, 24u32] (K16-major).  Stage the CTA's (output_n_tile,
            # k-tile) block of tiles into SMEM as [kt_local, nt16_local, 24u32]
            # contiguous.  Per kt_local the CTA's N16-tile range is contiguous in
            # global (24*cta_n_blocks u32), but consecutive kt_local jump by the
            # full (N/16)*24 row stride, so we copy per-kt contiguous n-spans in a
            # flat 16-byte cp.async loop (chunk c -> kt_local = c // chunks_per_kt).
            # SMEM byte c*16 == the register read's tile_base_u32*4 + word*4, so
            # the per-lane 2-word read indexes it directly.
            t256_n16 = self.size_n // 16
            t256_expert_u32 = (self.size_k // 16) * t256_n16 * 24
            t256_chunks_per_kt = self.cta_n_blocks * 6  # cta_n_blocks*24 // 4
            ...
            for i in cutlass.range_constexpr(self.b_sh_wr_iters_nf3):
                ...
                if cutlass.const_expr(self.weight_layout_trellis256_proj):
                    # Shared projection-major FC1 backing:
                    #   [2, E, K/16, (N/2)/16, 24u32]
                    # Logical output N is still gate|up.  Each 256-wide CTA is
                    # wholly inside one projection (validated in __init__), so
                    # select the plane then reuse the native per-projection map.
                    ...
                    t256_proj_expert_u32 = (
                        (self.size_k // 16) * t256_half_n16 * 24
                    )
                    ...
                    b_src_i32 = (
                        ...
                        * Int32(t256_half_n16 * 24)
                        + t256_local_n16 * Int32(24)
                        + t256_in_kt * Int32(4)
                    )
                else:
                    b_src_i32 = (
                        ...
                        + (tile_idx * Int32(self.cta_k_blocks) + t256_kt)
                        * Int32(t256_n16 * 24)
                        + output_n_tile * Int32(self.cta_n_blocks * 24)
                        + t256_in_kt * Int32(4)
                    )
                cp_async4_shared_global_pred(
                    b_dst,
        ...
        if cutlass.const_expr(self.weight_layout_nf3):
            ...
            # output_n_tile, k-stage) B block out as one contiguous
            # 12-byte-triple span, copied verbatim in 16-byte chunks.
            # b_gl_rd_base is unused on this path.
        ...
        for i in cutlass.range_constexpr(
            0 if self.b_region_12byte else self.b_sh_wr_iters
        ):
            b_src_int4 = (
                b_gl_rd_base
    ...
    @cute.jit
    def _prefetch_nf3_l2_lookahead(
        self,
        b_i32_flat: cute.Tensor,
        tid: Int32,
        tile_idx: Int32,
        ...
        if cutlass.const_expr(self.l2_prefetch and self.weight_layout_nf3):
            fetch_tile = tile_idx + Int32(self.stages + 1)
            if fetch_tile < k_tiles and tid < Int32(48):
                nf3_units_per_expert = (self.size_n * self.size_k) // 32
                nf3_ntile_stride = (self.size_k // 16) * (self.tile_n // 2)
                nf3_abs_tile = reduce_k_tile + fetch_tile
                nf3_span_base_unit = (
                    Int32(nf3_units_per_expert) * expert_idx
                    + Int32(nf3_ntile_stride) * output_n_tile
                    + nf3_abs_tile * Int32(self.cta_k_blocks * self.b_sh_stride)
                )
                # One 128-byte line per participating thread covers a 6144-byte
                # NF3 stage at the pinned 256x64 decode geometry.
                b_src_i32 = nf3_span_base_unit * Int32(3) + tid * Int32(32)
                prefetch_global_l2(get_ptr_as_int64(b_i32_flat, b_src_i32))
    ...
    def _prefetch_pipeline_step(
        ...
        if cutlass.const_expr(kk == self.b_sh_wr_iters - 2):
            ...
            if cutlass.const_expr(self.l2_prefetch and self.weight_layout_nf3):
                self._prefetch_nf3_l2_lookahead(
                    b_i32_flat,
                    tid,
                    tile_idx,
                    k_tiles,
    ...
    def _prefetch_initial_tiles(
        ...
        for pipe in cutlass.range_constexpr(self.stages - 1):
            if Int32(pipe) < k_tiles:
                ...
                cute.arch.cp_async_commit_group()
        cute.arch.cp_async_wait_group(self.cp_async_wait_groups)
        cute.arch.sync_threads()
...
class W4A16FusedMoeKernel:
    def __init__(
        ...
        *,
        ...
        self.direct_topk_routes = bool(direct_topk_routes)
        self.schedule_whole_tiles = bool(schedule_whole_tiles)
        # GATE 5b unblock: rotation-aware activation epilogue.  When set, the
        # activation pass (between the FC1 and FC2 grid barriers) applies, per
        # routed row, the exl3/QTIP per-expert blockwise-128 Hadamard incoherence
        # rotations on the I=intermediate_size dim that straddle silu:
        #   ig2 = had128(gate)*svh_gate ; iu2 = had128(up)*svh_up   (post-scale, pre-silu)
        #   ia2 = had128(silu(ig2)*iu2 * suh_down)                  (pre-scale,  post-silu)
        # The per-route [3*I] fp16 scales (svh_gate|svh_up|suh_down) are supplied
        # at launch in rot_scales_flat.  Byte-identical to the plain silu epilogue
        # when False (const_expr gated).  Only the trellis3_t256 tail needs it.
        ...
        if self.intermediate_rotation:
            ...
            if not is_gated or self.activation_is_swigluoai or self.has_swiglu_limit:
                raise ValueError(
                    "intermediate_rotation requires plain gated silu (no swiglu limit/oai)"
                )
        ...
        self.fc2 = W4A16GemmKernel(
            ...
            weight_layout=weight_layout,
            scale_format=scale_format,
            # Projection-major storage applies only to fused gate|up FC1.  The
            # down tensor remains the admitted native per-expert layout.
...
def compile_w4a16_fused_moe(
    *,
    ...
    # wired (per-warp native [K/16,N/16,24u32] tile staging + the per-lane 2-word
    # funnel read) and ADMITTED against a full-GEMM numeric oracle (megakernel
    # FC1->silu->FC2 vs a torch reference over numpy-decoded tiles;
    ...
    l2_prefetch = bool(l2_prefetch)
    if pipeline_stages < 4 or pipeline_stages > 8:
        raise ValueError(f"unsupported W4A16 pipeline_stages={pipeline_stages}")
    if l2_prefetch and not (
        weight_layout == "nf3_2p1" and pipeline_stages > 4
    ):
        raise ValueError("B12X_NF3_DRAM_HIDE_L2 requires NF3 pipeline_stages > 4")
    ...
    if pipeline_stages > _STAGES and kernel.shared_words * 4 > int(max_shared_mem) - 512:
        raise ValueError(
            "W4A16 widened-stage launch exceeds guarded shared-memory budget: "
            f"{kernel.shared_words * 4} > {int(max_shared_mem) - 512} bytes "
            f"(stages={pipeline_stages}, layout={weight_layout})"
        )
    cache_key = (
        "w4a16_fused_moe",
        device,
    ...
    elif weight_layout in ("nf3_2p1", "trellis3_t256"):
        # NF3 uses three int32 words per 32-code unit; Trellis256 uses 24 int32
        # words per 256-code tile. Both have the identical total footprint
        # K*N*3/32; only the in-tensor layout and staging differ.
        w13_fake = cute.runtime.make_fake_compact_tensor(
            cutlass.Int32,
    ...
    # Per-route [svh_gate|svh_up|suh_down] fp16 intermediate-rotation scales
    # (only read when intermediate_rotation; a tiny dummy is passed otherwise).
    rot_scales_fake = cute.runtime.make_fake_compact_tensor(
        cutlass.Float16,
        (max(compile_routed_rows * 3 * intermediate_size, 1),),
        assumed_align=16,
    )
...
def run_w4a16_moe(
    ...
    *,
    ...
    if _intermediate_rotation:
        need = int(m) * topk * 3 * intermediate_size
        ...
        if int(rot_arg.numel()) < need or rot_arg.dtype != torch.float16:
            raise ValueError(
                ...
                f"{need} elements (routes*3*intermediate); got "
    ...
    if collect_activation_amax:
    ...
    elif _intermediate_rotation:
        # Rotation-aware path bypasses the registered torch op (which stays
        # byte-identical for the production nf3/l32/packed/modelopt arms) and
        # calls the flat launch directly with the extra rot_scales tensor.
        (
        ...
        _w4a16_fused_moe_launch_flat(
            a_input=_rc_a, w13_arg=_rc_w13, w2_arg=_rc_w2, fc1_out=_rc_fc1,
            ...
            w13_layout=_lt_w13l, direct_topk_routes=bool(use_direct_topk_routes),
            fc1_tile_k=_lt_fc1tk, fc1_tile_n=_lt_fc1tn,
            fc2_tile_k=_lt_fc2tk, fc2_tile_n=_lt_fc2tn,
            tc_decode_fused_sum=bool(use_tc_decode), collect_activation_amax=False,
            stream_int=int(stream), rot_scales=rot_arg, intermediate_rotation=True,
        )
    else:
